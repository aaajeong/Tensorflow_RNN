...
train = tf.train.GradientDescentOptimizer(learning_rate=learning_rate).minimize(mean_loss)
...
for i in range(500):
    _, l, results = sess.run(
        [train, mean_loss, prediction], feed_dict={X: dataX, Y: dataY})
...
'''
실행이 잘 안됨
이유 : logits = output 과 관련이 있다.
해결과정은 lab-12-4-rnn_long_char(2).py 파일
'''
